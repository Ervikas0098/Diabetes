...
function simulateRF(data) {
  let logOdds = -1.2; // intercept (calibrated)

  // Polyuria & Polydipsia are top predictors (importance 0.22, 0.18)
  if (data.Polyuria === 1)          logOdds += 1.8;
  if (data.Polydipsia === 1)        logOdds += 1.6;
  if (data.Age > 50)                logOdds += 1.1;
  else if (data.Age > 40)           logOdds += 0.7;
  else if (data.Age > 30)           logOdds += 0.3;
  if (data["sudden weight loss"]===1) logOdds += 0.9;
  if (data.Gender === 1)            logOdds += 0.5; // Male slightly higher risk
  if (data.Polyphagia === 1)        logOdds += 0.7;
  if (data.Irritability === 1)      logOdds += 0.6;
  if (data["partial paresis"]===1)  logOdds += 0.6;
  if (data.weakness === 1)          logOdds += 0.4;
  if (data["visual blurring"]===1)  logOdds += 0.4;
  if (data["delayed healing"]===1)  logOdds += 0.35;
  if (data["muscle stiffness"]===1) logOdds += 0.3;
  if (data.Alopecia === 1)          logOdds += 0.25;
  if (data["Genital thrush"]===1)   logOdds += 0.35;
  if (data.Itching === 1)           logOdds += 0.2;
  if (data.Obesity === 1)           logOdds += 0.5;

  const prob = 1 / (1 + Math.exp(-logOdds));
  const score = Math.round(Math.min(98, Math.max(3, prob * 100)));
  ...
  const contributions = [
    ...
    { name:"Age",                              val: data.Age>50?14:data.Age>40?9:data.Age>30?4:1, present: true },
    { name:"Sudden Weight Loss",               val: data["sudden weight loss"]*10,  present: data["sudden weight loss"]===1 },
    { name:"Gender",                           val: data.Gender*6,          present: data.Gender===1 },
    { name:"Polyphagia (Excess Hunger)",       val: data.Polyphagia*7,      present: data.Polyphagia===1 },
    { name:"Irritability",                     val: data.Irritability*6,    present: data.Irritability===1 },
    { name:"Partial Paresis",                  val: data["partial paresis"]*6, present: data["partial paresis"]===1 },
    { name:"Weakness",                         val: data.weakness*5,        present: data.weakness===1 },
    { name:"Obesity",                          val: data.Obesity*5,         present: data.Obesity===1 },
    { name:"Visual Blurring",                  val: data["visual blurring"]*4, present: data["visual blurring"]===1 },
    { name:"Delayed Healing",                  val: data["delayed healing"]*4, present: data["delayed healing"]===1 },
    { name:"Muscle Stiffness",                 val: data["muscle stiffness"]*3, present: data["muscle stiffness"]===1 },
    { name:"Genital Thrush",                   val: data["Genital thrush"]*4,  present: data["Genital thrush"]===1 },
    { name:"Alopecia (Hair Loss)",             val: data.Alopecia*3,        present: data.Alopecia===1 },
    { name:"Itching",                          val: data.Itching*2,         present: data.Itching===1 },
